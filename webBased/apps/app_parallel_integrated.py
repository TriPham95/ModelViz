...
# -*- coding: utf-8 -*-
"""
Created on Sat Mar 14 12:19:29 2020
...
def build_banner():
    return html.Div(
        ...
        children=[
            html.H2("PARALLEL INTEGRATED FILE VISUALIZATION"),
        ],
    )
...
# Original Map
@app.callback(
        Output(component_id='integrated-basin2', component_property='figure'),
...
def Update_Main_Map(style, filename_voronoi, contents_voronoi,  
    ...
    if contents_voronoi and contents_integrated:
        if len(contents_voronoi) == len(contents_integrated):
            # Processing voronoi data
            voronoi_file = [parse_voronoi_file(x, y) for x, y in zip(contents_voronoi, filename_voronoi)]
            voronoi_polygons = [item for sublist in voronoi_file for item in sublist]
            #print(voronoi_polygons) 

            # Processing integrated file
            integrated_file = [parse_integrated_file(x, y) for x, y in zip(contents_integrated, filename_integrated)]
        
            integrated_variables = pd.concat(integrated_file)
            integrated_variables = integrated_variables.reset_index(drop=True)
            #print(integrated_file)

        
            #spatial_data = process_integrated_file(integrated_file)
            # Merging voronoi and spatial data
            gdf = merging_voronoi_integrated(voronoi_polygons, integrated_variables, UTM_Option)
